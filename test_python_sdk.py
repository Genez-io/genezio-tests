...
def test_python_sdk():
    ...
    assert "def method():" in content, "Wrong exported method without parameters"
    assert 'return Server.remote.call("Server.method")' in content, "Wrong exported method without parameters"
    
    assert 'def methodWithoutParameters():' in content, "Wrong exported method with return type"
    assert 'return Server.remote.call("Server.methodWithoutParameters")' in content, "Wrong exported method with return type"
    
    assert 'def methodWithOneParameter(test1):' in content, "Wrong exported method with one parameter"
    assert 'return Server.remote.call("Server.methodWithOneParameter", test1)' in content, "Wrong exported method with one parameter"
    
    assert 'def methodWithMultipleParameters(test1, test2):' in content, "Wrong exported method with multiple parameters"
